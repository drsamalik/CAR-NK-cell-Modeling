...
class Main_Model():
    ...
    def Solve_eqn(self, Tc_No):
        ...
        return yF
    def Tumor(self, a1, w, alpha4, NK_No, Tc_No):
...
def Rho_lambda(a1,alpha4, NK_cell,Target_cell, WT = False):
        x0 = np.array([0.9791789663709559,6141.439265755344,0.9998746637454399,0.8936323663149114,0.13167866188293606,0.3544511431834766,
                        0.007823220814404435,3.684176975019324e-05])
        x = np.array(x0)
...
def R_H_Expression(NK_cell,frac):
    ...
    dat0 = pd.read_excel('/Users/sxa126/Dropbox/CAR_NK_model/CAR_NK_Python/Pareto_Opt/Pure_Mono/data/Kasumi1_CAR_NK_CD33.xlsx')
    dat10 = pd.read_excel('/Users/sxa126/Dropbox/CAR_NK_model/CAR_NK_Python/Pareto_Opt/Pure_Mono/data/Kasumi1_LFA1_ICAM1.xlsx',sheet_name=0)
    dat11 = pd.read_excel('/Users/sxa126/Dropbox/CAR_NK_model/CAR_NK_Python/Pareto_Opt/Pure_Mono/data/Kasumi1_LFA1_ICAM1.xlsx',sheet_name=1)
    dat2 = pd.read_excel('/Users/sxa126/Dropbox/CAR_NK_model/CAR_NK_Python/Pareto_Opt/Pure_Mono/data/Kasumi1_KIR2DL1_HLA.xlsx')
    if NK_cell == 'CAR_NK':
        R1_ER1 = CAR_data(frac)#dat0.loc[0:4,['No_of_CAR_NK','prob_Car_NK']].rename(columns= {'No_of_CAR_NK' : 'R1','prob_Car_NK' : 'T_R1'}, inplace = False)
...
def CAR_data(frac=0.54):
    ...
    param = np.array([0.010000056400174168, 1.3610738915360896, 2.673961057813516, 1.1375219647613837])
