...
class Model_obj_CAR_NK:
    ...
    def Cell_type_R_H(self,E=1, cell_type=None,mult=1):
        ...
        self.NK_cell = pd.DataFrame(columns=['T_R', 'R1', 'R4'])
        Effector = E
        for index1, row1 in R[0].iterrows():
                for index4, row4 in R[2].iterrows():
                    T_R = (row1['T_R1']*row4['T_R4']) * Effector
                    self.NK_cell.loc[len(self.NK_cell)] = [T_R, row1['R1'], row4['R4']]
        Target = 10000.0
        self.Target_cell = pd.DataFrame(columns=['U_H', 'H1', 'H4'])
        for index1, row1 in H[0].iterrows():
                for index4, row4 in H[2].iterrows():
                    U_H = (row1['U_H1']*row4['U_H4']) * Target
                    self.Target_cell.loc[len(self.Target_cell)] = [U_H, row1['H1'], row4['H4']]
    def Rho_Lambda(self,x0):
        R_set = self.NK_cell[['R1', 'R4']]
        H_set = self.Target_cell[['H1', 'H4']]
        # Unknown para
        # R1 H1
        alph1 = x0[0]
        # Activating
    ...
    def Effector_vs_Lysis(self,x0):
        ...
        with Pool() as pool:
            Specific_lysis = pool.map(Model_obj_CAR_NK.worker, args_list)
