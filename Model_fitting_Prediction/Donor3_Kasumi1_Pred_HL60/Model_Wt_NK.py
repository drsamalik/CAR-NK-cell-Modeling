...
class Model_obj_WT_NK:
    def __init__(self):
        input_const = np.array([0.0165, 1.0])
        x = input_const*3600 # par hour
    ...
    def Cell_type_R_H(self,E=1, cell_type=None):
        ...
        self.NK_cell = pd.DataFrame(columns=['T_R', 'R3', 'R4'])
        Effector = E
        for index3, row3 in R[1].iterrows():
            for index4, row4 in R[2].iterrows():
                T_R = (row3['T_R3']*row4['T_R4']) * Effector
                self.NK_cell.loc[len(self.NK_cell)] = [T_R, row3['R3'], row4['R4']]
        Target = 10000.0
        self.Target_cell = pd.DataFrame(columns=['U_H', 'H3', 'H4'])
        for index3, row3 in H[1].iterrows():
            for index4, row4 in H[2].iterrows():
                U_H = (row3['U_H3']*row4['U_H4']) * Target
                self.Target_cell.loc[len(self.Target_cell)] = [U_H, row3['H3'], row4['H4']]
    def Rho_Lambda(self,x0):
        R_set = self.NK_cell[['R3', 'R4']]
        H_set = self.Target_cell[['H3', 'H4']]
        C2N = x0[1]
        # R3 H3
        alph3 = (x0[2])#** self.N3
        # R4 H4
